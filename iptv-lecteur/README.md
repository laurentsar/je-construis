...
- **APK** : chaque changement publié dans `iptv-lecteur/` déclenche une
  Release GitHub signée, taguée `iptv-vX.Y`. L'app vérifie elle-même s'il en
...
secrets GitHub `ANDROID_KEYSTORE_B64` / `ANDROID_KEYSTORE_PASSWORD` (partagés
avec les autres apps du dépôt).
